...
const templatingWithMustache = () => {
  return characters.map((character) => {
    let mustachTemplate = $("#template").html();
    return  Mustache.render(mustachTemplate, character);
  })
};
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(({ house }) => houses.push(house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  const names = Object.values(arr).map((char) => char.name);
  return (names.includes(character));
};
