...
const templateWithJQuery = () => {
  // Solution code here...
  starWarsPeople.forEach(({ name, height, eye_color }) => {
    const template = $('#template').clone();
    template.removeAttr('id');
    template.find('h2').text(name);
    template.find('h3').text(height);
    template.find('p').text(eye_color);
...
const listFoods = (recipe) => {
  ...
  for (var i = 0; i < ingredients.length; i++) {
    var str = ingredients[i];
    var index = str.indexOf(' ', str.indexOf(' ') + 1);
    result[i] = str.slice(index + 1);
  }
  return result;
};
